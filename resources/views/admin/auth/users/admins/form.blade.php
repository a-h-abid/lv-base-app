...
@php
    $page_title = ucfirst($form_mode) . ' Admin';
    $method = $form_mode == 'create' ? 'post' : 'put';
    $route = $form_mode == 'create' ? 'admin.users.admins.store' : 'admin.users.admins.update';
    $route_params = $form_mode == 'create' ? [] : [$admin->id];
@endphp
...
@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ route('admin.users.admins.index') }}">Users Management -> Admins</a></li>
    ...
    <div class="card-body">
        ...
        <form action="{{ route($route, $route_params) }}" method="post">
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-6">
                    <input type="text" name="name" class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" id="name" placeholder="Name" value="{{ old('name') ?? $admin->name }}">
                    @if ($errors->has('name'))
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-6">
                    <input type="email" name="email" class="form-control {{ $errors->has('email') ? 'is-invalid' : '' }}" id="email" placeholder="E-mail" value="{{ old('email') ?? $admin->email }}">
                    @if ($errors->has('email'))
                    <div class="invalid-feedback">
                        {{ $errors->first('email') }}
                    </div>
            ...
            <div class="form-group row">
                <div class="offset-sm-3 col-sm-6">
                    <div class="checkbox">
                        <label>
                            <input type="checkbox" value="1" id="active" name="active" {{ (old('active') ?? $admin->active) == 1 ? 'checked' : '' }} > Active
                        </label>
                    </div>
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-6">
                    <select class="form-control {{ $errors->has('roles') ? 'is-invalid' : '' }}" name="roles[]" id="roles" multiple>
                        ...
                        <option value="{{ $roleId }}" {{ (in_array($roleId, old('roles') ?? $admin->roles->pluck('id')->toArray())) ? 'selected' : '' }} >{{ $roleName }}</option>
                        @endforeach
                    </select>
                    @if ($errors->has('roles'))
                    <div class="invalid-feedback">
